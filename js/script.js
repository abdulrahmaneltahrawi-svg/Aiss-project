...
function activateHeader() {
  ...
  if (searchInput) {
    ...
    searchInput.addEventListener("input", async function () {
      ...
      const blogs = typeof allBlogs !== "undefined" ? allBlogs : (typeof myCards !== "undefined" ? myCards : []);
      const eventsData = typeof events !== "undefined" ? events : [];
      const codes = typeof fetchCodes === "function" ? await fetchCodes() : [];

      const allItems = [
        ...(typeof magazines !== "undefined" ? magazines : []).map((m) => ({
          ...m,
          type: "مجلة",
          url: (m.file_path || m.link)
            ? `flipbook.html?title=${encodeURIComponent(m.title)}&src=${encodeURIComponent(m.file_path || m.link)}&back=${window.location.pathname.split("/").pop() || "index.html"}`
            : "#",
        })),
        ...(typeof manuals !== "undefined" ? manuals : []).map((m) => ({
          ...m,
          type: "كتيب",
          url: (m.file_path || m.link)
            ? `flipbook.html?title=${encodeURIComponent(m.title)}&src=${encodeURIComponent(m.file_path || m.link)}&back=${window.location.pathname.split("/").pop() || "index.html"}`
